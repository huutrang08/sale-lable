...
export default function OrdersTab() {
  ...
  function StatusBadge({ id }: { id: string }): JSX.Element {
    ...
    if (id === 'PROCESSING') return <span className="inline-flex items-center gap-1 bg-amber-50 text-amber-600 border border-amber-200 px-2 py-0.5 rounded-md text-xs font-bold">⏳ PROCESSING</span>;
  ...
  function renderDetailModal(d: Order): JSX.Element {
        ...
        <div className={`-mx-8 -mt-8 mb-6 px-8 pt-8 pb-6 rounded-t-3xl ${isFailed ? 'bg-gradient-to-br from-red-500 to-rose-600' : isProcessing ? 'bg-gradient-to-br from-amber-400 to-orange-500' : 'bg-gradient-to-br from-indigo-600 via-violet-600 to-purple-700'}`}>
          <div className="flex items-start justify-between gap-4">
            <div>
              ...
              {isFailed ? <span className="inline-flex items-center gap-1.5 bg-white/20 text-white border border-white/30 px-3 py-1 rounded-full text-sm font-black">❌ FAILED</span> : null}
              {isProcessing ? <span className="inline-flex items-center gap-1.5 bg-white/20 text-white border border-white/30 px-3 py-1 rounded-full text-sm font-black">⏳ PROCESSING</span> : null}
              {hasTracking ? (
          ...
          <div className="flex items-stretch gap-0">
            <div className="flex-1 min-w-0">
              ...
              <div className="text-xs text-slate-500 leading-relaxed">
                {d.from_address ? <div>{d.from_address}</div> : null}
            ...
            <div className="flex-1 min-w-0">
              ...
              <div className="text-xs text-slate-500 leading-relaxed">
                {d.to_address ? <div>{d.to_address}</div> : null}
  ...
  return (
    <Card title="📦 Transaction History" className="p-0 sm:p-0">
      {/* Toolbar */}
              ...
              <tr key={o.id} className="hover:bg-slate-50/80 transition-colors group align-middle">
                ...
                <td className="px-5 py-4 min-w-[200px]">
                  <div className="flex items-start gap-2">
                    <div className="flex-1 min-w-0">
                      <div className="text-xs text-slate-400 font-semibold mb-0.5">FROM</div>
                    ...
                    <div className="flex-1 min-w-0">
                      <div className="text-xs text-slate-400 font-semibold mb-0.5">TO</div>
                  ...
                  <div className="flex items-center justify-end gap-1.5">
                    {o.pdf && (
                      <a href={o.pdf} target="_blank" rel="noreferrer"
                        className="inline-flex items-center justify-center w-8 h-8 bg-slate-100 text-slate-600 hover:bg-emerald-100 hover:text-emerald-700 rounded-lg transition-colors text-sm" title="Print Label">
                        🖨️
                      </a>
                    )}
                    <button onClick={() => setDetail(o)}
                      className="inline-flex items-center justify-center w-8 h-8 bg-slate-100 text-slate-600 hover:bg-blue-100 hover:text-blue-700 rounded-lg transition-colors text-sm" title="Details">
                      🔍
                    </button>
                  </div>
                </td>
